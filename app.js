...
function save(forceTab){const tab=forceTab||currentOpenTab();localStorage.setItem(KEY,JSON.stringify(state));render(tab)}
...
function navIcon(id){
  const common='class="nav-icon" viewBox="0 0 24 24" fill="none" aria-hidden="true"';
  const icons={
    train:`<svg ${common}><path d="M6.5 9.25v5.5M17.5 9.25v5.5M4.5 10.2v3.6M19.5 10.2v3.6M8.2 12h7.6" stroke="currentColor" stroke-width="1.9" stroke-linecap="round"/><path d="M6.5 8.4h1.7v7.2H6.5a1.1 1.1 0 0 1-1.1-1.1v-5a1.1 1.1 0 0 1 1.1-1.1ZM15.8 8.4h1.7a1.1 1.1 0 0 1 1.1 1.1v5a1.1 1.1 0 0 1-1.1 1.1h-1.7V8.4Z" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"/></svg>`,
    diet:`<svg ${common}><path d="M12 18.2a6.2 6.2 0 1 0 0-12.4 6.2 6.2 0 0 0 0 12.4Z" stroke="currentColor" stroke-width="1.7"/><path d="M12 15.4a3.4 3.4 0 1 0 0-6.8 3.4 3.4 0 0 0 0 6.8Z" stroke="currentColor" stroke-width="1.4" opacity=".82"/><path d="M3.7 5.8v7.4M5.2 5.8v7.4M6.7 5.8v7.4M5.2 13.2v5.2M20.1 5.8v12.6M18.5 5.9c1.5.9 1.7 3.6.2 5.2" stroke="currentColor" stroke-width="1.45" stroke-linecap="round"/></svg>`,
    calendar:`<svg ${common}><path d="M7 3.9v3M17 3.9v3M5.4 6.2h13.2a1.8 1.8 0 0 1 1.8 1.8v10.1a1.8 1.8 0 0 1-1.8 1.8H5.4a1.8 1.8 0 0 1-1.8-1.8V8a1.8 1.8 0 0 1 1.8-1.8Z" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/><path d="M3.9 9.6h16.2" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/><path d="M7.2 12.5h.1M11.9 12.5h.1M16.6 12.5h.1M7.2 16h.1M11.9 16h.1M16.6 16h.1" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"/></svg>`,
    progress:`<svg ${common}><path d="M4.2 16.8l4.7-4.7 3.3 3.1 7.6-8" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"/><path d="M15.9 7.2h3.9v3.9" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"/><path d="M5.2 19.2h14.2M6.7 15.4v3.8M11.8 14.2v5M17.1 10.6v8.6" stroke="currentColor" stroke-width="1.45" stroke-linecap="round" opacity=".78"/></svg>`,
    more:`<svg ${common}><path d="M4.5 7.2h15M4.5 12h15M4.5 16.8h15" stroke="currentColor" stroke-width="1.55" stroke-linecap="round"/><path d="M9 7.2a1.8 1.8 0 1 0 0-.01ZM15 12a1.8 1.8 0 1 0 0-.01ZM11.5 16.8a1.8 1.8 0 1 0 0-.01Z" fill="currentColor"/></svg>`
  };
  return icons[id]||"";
}

function shell(){let p=prof();document.getElementById("app").innerHTML=`<div class="shell"><header class="topbar"><div class="title-row"><div class="brand"><div class="logo"><img src="./icon-192.png?v=14" alt="Sheipados" onerror="this.parentElement.outerHTML=\'<div class=&quot;logo-fallback&quot; aria-label=&quot;Sheipados&quot;></div>\'"></div><div><h1>${PLAN.project.name}</h1><div class="sub">${p.name}</div></div></div><select class="profile-select" onchange="switchProfile(this.value)">${Object.entries(PLAN.profiles).map(([k,v])=>`<option value="${k}" ${state.profile===k?"selected":""}>${v.name}</option>`).join("")}</select></div></header><main class="content">${tabs.map(t=>`<section class="section" id="${t[0]}"></section>`).join("")}</main><nav class="nav">${tabs.map(t=>`<button data-tab="${t[0]}" onclick="openTab('${t[0]}')"><span class="ico">${navIcon(t[0])}</span><span>${t[1]}</span></button>`).join("")}</nav></div>`}
...
function checkinTimeText(obj){
  return obj?.checkInTime ? `Registrado às ${obj.checkInTime}` : "";
}

function todayLog(){
  return u().dailyLogs.find(x=>x.date===today());
}
function isWorkoutDoneToday(){
  const log=todayLog();
  return !!(log?.workout?.saved && log?.workout?.complete);
}
...
function workoutDoneSummary(log){
...
function editDietToday(){
  u().editingDietDate=today();
  save();
}
...
function exerciseOption(e,i){
  const sel=$(`ex-choice-${i}`);
...
function renderTrain(){let existing=todayLog();if(isWorkoutDoneToday()&&u().editingWorkoutDate!==today()){$("train").innerHTML=`${head("Hoje • Treino","Check-in registrado.")}${completedCard("train","Treino concluído hoje","Treino registrado no calendário de evolução.",workoutDoneSummary(existing),"editWorkoutToday()")}`;return}let w=workout(), wk=week(), aw=activeWeekNumber(), done=weekProgressInCycle(), total=coreWorkoutCheckins();$("train").innerHTML=`${head("Hoje • Treino",`Detectado automaticamente: ${activeDay()}.`)}<div class="grid two"><div class="card kpi"><div class="label">Treino</div><div class="value" style="font-size:20px">${w.title}</div><div class="hint">${activeDay()}</div></div><div class="card kpi"><div class="label">Semana automática</div><div class="value">S${aw}</div><div class="hint">${done}/5 check-ins nesta semana</div></div></div><div class="card"><p class="pill">${currentWorkoutBlock().name} • ${currentWorkoutBlock().focus}</p><p class="notice"><b>Ciclo:</b> início em 29/06/2026. A cada 5 check-ins válidos de segunda a sexta, o app avança automaticamente uma semana. Sábado/domingo fica como bônus opcional e não conta para avançar semana.</p><p class="muted" style="font-size:13px">Check-ins válidos desde o início: ${total}. Semana atual: S${aw} • ${wk.phase}.</p><details><summary>Mudar treino só hoje</summary><div class="form-grid"><div><label>Semana só hoje</label><select onchange="u().weekOverrideDate=today();u().weekOverride=Number(this.value);save()">${PLAN.weeks.map(x=>`<option value="${x.week}" ${x.week===aw?"selected":""}>Semana ${x.week}</option>`).join("")}</select></div><div><label>Dia só hoje</label><select onchange="u().dayOverrideDate=today();u().dayOverride=this.value;save()">${Object.keys(workouts()).map(d=>`<option ${d===activeDay()?"selected":""}>${d}</option>`).join("")}</select></div></div><div class="actions"><button class="ghost" onclick="u().weekOverrideDate=null;u().weekOverride=null;u().dayOverrideDate=null;u().dayOverride=null;save()">Limpar troca de hoje</button></div></details><h3>Exercícios executados</h3><p class="muted" style="font-size:13px">Escolha a variação realmente executada. A prescrição mostra séries, repetições, carga por esforço, descanso, tempo e vídeo curado.</p><div class="list">${w.exercises.map((e,i)=>exerciseCard(e,i)).join("")}</div><div style="margin-top:12px"><label>Nota do treino</label><textarea id="tr-note" placeholder="Ex.: completo, energia baixa, adaptação..."></textarea></div><div class="actions"><button class="primary" onclick="saveTrain()">Registrar treino</button></div></div>`}
function saveTrain(){let w=workout(), aw=activeWeekNumber(), ex=w.exercises.map((e,i)=>{let opt=exerciseOption(e,i);return {name:opt.name,base:e.name,done:true,sets:opt.sets,reps:opt.reps,effort:opt.effort,load:opt.load,rest:opt.rest,video:opt.video}});let existingIdx=u().workoutLogs.findIndex(l=>l.date===today());let checkInTime=nowTime(), checkInAt=nowISO();let payload={id:existingIdx>=0?u().workoutLogs[existingIdx].id:uid(),date:today(),week:aw,day:activeDay(),title:w.title,exercises:ex,note:$("tr-note").value,complete:true,checkInTime,checkInAt};if(existingIdx>=0)u().workoutLogs[existingIdx]=payload;else u().workoutLogs.push(payload);u().editingWorkoutDate=null;upsertDaily({exercises:ex,day:activeDay(),week:aw,workoutTitle:w.title,workout:{saved:true,complete:true,count:ex.length,total:ex.length,checkInTime,checkInAt}});save("train")}
...
function progressFlags(date){
  ...
  const trainDaily=!!(daily?.workout?.complete || ((daily?.exercises||[]).length>0 && (daily.exercises||[]).every(e=>e.done)));
  ...
  const trainLog=(u().workoutLogs||[]).some(l=>l.date===date && (l.complete || ((l.exercises||[]).length>0 && (l.exercises||[]).every(e=>e.done))));
...
async function extractImageText(file,statusEl){
  if(!window.Tesseract) throw new Error("OCR não carregado. Verifique a internet e atualize a página.");
  const result=await Tesseract.recognize(file,"por+eng",{
    logger:m=>{
      if(statusEl && m.status){
        const pct=m.progress?` ${Math.round(m.progress*100)}%`:"";
        statusEl.textContent=`Lendo imagem: ${m.status}${pct}`;
      }
    }
  });
...
function latestBio(){
  return [...(u().bodyLogs||[])]
    .filter(x=>x.source==="bioimpedance_pdf"||x.source==="bioimpedance_image")
...
async function saveBioFromFile(file, mode){
  ...
  if(status) status.textContent=mode==="pdf" ? "Lendo PDF de bioimpedância..." : "Lendo foto/imagem de bioimpedância...";
  try{
    const text=mode==="pdf" ? await extractPdfText(file) : await extractImageText(file,status);
    const parsed=parseBioimpedanceText(text);
    const hasAny=Object.values(parsed).some(v=>!!v);
    if(!hasAny){
      if(status) status.textContent="Arquivo lido, mas não encontrei valores de bioimpedância automaticamente.";
      alert("Li o arquivo, mas não identifiquei os valores automaticamente. Tente um PDF com texto selecionável ou uma foto mais nítida.");
      return;
    }
    const date=today();
    const rec={id:uid(),date,source:mode==="pdf"?"bioimpedance_pdf":"bioimpedance_image",filename:file.name,size:file.size,...parsed,createdAt:new Date().toISOString()};
    u().bodyLogs.push(rec);
    if(status) status.textContent="Bioimpedância salva.";
    save();
  }catch(err){
    console.error(err);
