...
const PLAN=window.PLAN_DATA, KEY="sheipados_v38";
...
function workoutDoneSummary(log){
  ...
  return `<div class="done-summary"><span>Exercícios concluídos</span><strong>${done}/${total}</strong></div><p class="muted" style="font-size:12px">Amanhã o app libera automaticamente o próximo treino do dia.</p>`;
}
function dietDoneSummary(log){
  return `<div class="done-summary"><span>Flags da dieta</span><strong>${log?.diet?.count||0}/${log?.diet?.total||0}</strong></div><p class="muted" style="font-size:12px">Amanhã o checklist da dieta será liberado novamente.</p>`;
}
...
function saveDiet(){markDietAll();let date=today(), flags=collectDietFlags();let existingIdx=u().dietLogs.findIndex(l=>l.date===date);let payload={id:existingIdx>=0?u().dietLogs[existingIdx].id:uid(),date,complete:true,meals:flags.meals.map(m=>({...m,done:true})),extras:flags.extras.map(e=>({...e,done:true})),water:flags.water.map(w=>({...w,done:true})),schedule:flags.schedule.map(s=>({...s,done:true})),note:$("diet-note").value};if(existingIdx>=0)u().dietLogs[existingIdx]=payload;else u().dietLogs.push(payload);u().editingDietDate=null;upsertDaily({date,meals:payload.meals,extras:payload.extras,water:payload.water,schedule:payload.schedule,diet:{saved:true,complete:true,count:flags.total,total:flags.total}});save("diet")}
